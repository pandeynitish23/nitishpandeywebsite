...
function initVisitorCounter() {
  ...
  // Realistic base offset so count looks credible from day one
  const BASE_COUNT = 120;
  let stored = parseInt(localStorage.getItem('nkp_visitor_count') || '0');

  // Increment on each new page load
  stored += 1;
  localStorage.setItem('nkp_visitor_count', stored);

  const displayCount = BASE_COUNT + stored;

  // Animate count-up using same easeOutQuart as hero stats
  const duration = 1800;
  const stepMs = 60;
  ...
  const timer = setInterval(() => {
    current++;
    const value = Math.round(easeOutQuart(current / steps) * displayCount);
    el.textContent = value;
    if (current >= steps) {
