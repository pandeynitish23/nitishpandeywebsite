...
/* =============================================
   VISITOR COUNTER (countapi.xyz – real cross-device tracking)
   ============================================= */
function initVisitorCounter() {
  const el = document.getElementById('visitorCount');
  if (!el) return;

  const CACHE_KEY = 'nkp_visitor_cache';     // localStorage: last known count
  const SESSION_KEY = 'nkp_counted_session';   // sessionStorage: already counted this tab?

  // Use countapi.xyz – free, CORS-friendly, persistent server-side counter
  const HIT_URL = 'https://api.countapi.xyz/hit/nitishkumarpandey-portfolio/page-visits';
  const GET_URL = 'https://api.countapi.xyz/get/nitishkumarpandey-portfolio/page-visits';

  // Animate from 0 → count
  function animateToCount(count) {
    const duration = 1400;
    const stepMs = 60;
    ...
    const timer = setInterval(() => {
      current++;
      el.textContent = Math.round(easeOutQuart(current / steps) * count).toLocaleString();
      if (current >= steps) {
  ...
  // Show cached value instantly when API is slow/unavailable
  function showCached() {
    const cached = localStorage.getItem(CACHE_KEY);
    if (cached) {
      animateToCount(parseInt(cached, 10));
    } else {
      el.textContent = '—';
    }
  }
  ...
  if (!alreadyCounted) {
    // First load in this browser session → increment server count
    fetch(HIT_URL)
      .then(r => r.json())
      .then(data => {
        const count = data.value || 1;
        localStorage.setItem(CACHE_KEY, count);   // cache for fallback
        sessionStorage.setItem(SESSION_KEY, '1'); // mark session as counted
        animateToCount(count);
      })
      .catch(() => showCached()); // API down? show last known count
  } else {
    // Same-session refresh → read without incrementing
    fetch(GET_URL)
      .then(r => r.json())
      .then(data => {
        const count = data.value || parseInt(localStorage.getItem(CACHE_KEY) || '1', 10);
        localStorage.setItem(CACHE_KEY, count);
        animateToCount(count);
      })
      .catch(() => showCached()); // API down? show cached count
  }
}
