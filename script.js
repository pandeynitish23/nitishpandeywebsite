...
function initContactForm() {
  ...
  const LINKEDIN_PROFILE = 'https://www.linkedin.com/messaging/thread/2-ZDNhNjBjNDktNzVhYS00YTc2LTg5MDEtNzczMWEwNjdkNGFiXzEwMA==/';
  const MY_EMAIL = 'nitishkumar.nk.np@gmail.com';

  let activeChannel = 'whatsapp';

  const allChannelBtns = [whatsappBtn, linkedInBtn, emailBtn];
  ...
  // Also add a dedicated Send button via JS if needed; for clarity add a real submit btn
  addSubmitButton(form);

  function addSubmitButton(form) {
    const existingBtn = document.getElementById('realSubmitBtn');
    if (existingBtn) return;

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.id = 'realSubmitBtn';
    btn.innerHTML = '<i class="fas fa-paper-plane"></i> <span>Send Message</span>';
    btn.className = 'btn-primary';
    btn.style.cssText = 'width:100%;justify-content:center;padding:1rem 2rem;font-size:1rem;border:none;cursor:pointer;';
