...
/* =============================================
   VISITOR COUNTER (CountAPI – real cross-device tracking)
   ============================================= */
function initVisitorCounter() {
  const el = document.getElementById('visitorCount');
  if (!el) return;

  // sessionStorage prevents re-counting on same-tab refresh
  const SESSION_KEY = 'nkp_counted_this_session';
  const alreadyCounted = sessionStorage.getItem(SESSION_KEY);

  const NAMESPACE = 'nitishkumarpandey-portfolio';
  ...
  function showFallback() { el.textContent = '...'; }
  ...
  if (!alreadyCounted) {
    ...
    fetch(HIT_URL)
      ...
      .then(data => {
        ...
        animateToCount(data.count || data.value || 1);
      })
      .catch(showFallback);
  } else {
    // Same session refresh → just read current count
    fetch(GET_URL)
      .then(r => r.json())
      .then(data => animateToCount(data.count || data.value || 1))
      .catch(showFallback);
  }
}
