...
/* =============================================
   ...
   To show an alert: set CLOUD_ALERT.message to a non-empty string.
   To hide it:       set CLOUD_ALERT.message to "" (empty string).
   ============================================= */
const CLOUD_ALERT = {
  // ← EDIT THIS TEXT to set an alert. Leave it "" to hide the banner.
  message: "https://timesofindia.indiatimes.com/technology/tech-news/amazons-datacentre-in-uae-reports-fire-company-says-at-around-430-am-pst-one-of-our-availability-zones-was-impacted-by-/articleshow/128937624.cms"
};
...
/* =============================================
   CLOUD ALERT BANNER
   ============================================= */
function initCloudAlert() {
  const wrapper = document.getElementById('cloudAlertWrapper');
  const textEl = document.getElementById('cloudAlertText');
  const closeBtn = document.getElementById('cloudAlertClose');
  if (!wrapper || !textEl || !closeBtn) return;

  const msg = (CLOUD_ALERT.message || '').trim();

  // If no message set, keep hidden and exit
  if (!msg) return;

  // If user already dismissed this exact message this session, stay hidden
  const dismissedMsg = sessionStorage.getItem('cloudAlertDismissed');
  if (dismissedMsg === msg) return;
  ...
  closeBtn.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cloudAlertDismissed', msg);
  });
}
